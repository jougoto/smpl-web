...
/** Базовый URL FastAPI (postback_app): http://IP:PORT или https://домен — без слэша в конце */
const SIGNALS_API_BASE = "http://2.27.122.20:8000";
...
const I18N = {
    en: {
        ...
        trading: {
            ...
            signalLimitReached: "Daily signal limit reached. Resets after 24 hours.",
            signalOpenInTelegram: "Open this app from Telegram to use signals.",
            signalLimitError: "Could not verify the limit. Try again later.",
        },
        picker: {
    ...
    ru: {
        ...
        trading: {
            ...
            signalLimitReached: "Дневной лимит сигналов исчерпан. Обновится через 24 часа.",
            signalOpenInTelegram: "Откройте приложение из Telegram.",
            signalLimitError: "Не удалось проверить лимит. Попробуйте позже.",
        },
        picker: {
    ...
    es: {
        ...
        trading: {
            ...
            signalLimitReached: "Límite diario alcanzado. Se renueva en 24 horas.",
            signalOpenInTelegram: "Abre la app desde Telegram.",
            signalLimitError: "No se pudo verificar el límite. Inténtalo más tarde.",
        },
        picker: {
    ...
    hi: {
        ...
        trading: {
            ...
            submit: "सिग्नल लो",
            dailyLimitLabel: "दैनिक लिमिट:",
            signalLimitReached: "दैनिक सिग्नल लिमिट पूरी। 24 घंटे बाद रीसेट।",
            signalOpenInTelegram: "कृपया Telegram से ऐप खोलें।",
            signalLimitError: "लिमिट जाँच नहीं हो सकी। बाद में कोशिश करें।",
        },
        picker: {
...
function applyLanguage() {
    ...
    if (window.jogotoRefreshDailySignalLimit) window.jogotoRefreshDailySignalLimit();
}
...
function showTradingNotice(msg) {
    const toast = document.querySelector("[data-vip-toast]");
    if (!toast || !msg) return;
    toast.textContent = msg;
    toast.classList.add("is-visible");
    if (vipToastTimer) clearTimeout(vipToastTimer);
    vipToastTimer = setTimeout(() => toast.classList.remove("is-visible"), 3200);
}

function getSignalsApiBase() {
    return String(SIGNALS_API_BASE || "").trim().replace(/\/+$/, "");
}

function getTelegramInitData() {
    try {
        const tw = window.Telegram && window.Telegram.WebApp;
        return tw && tw.initData ? String(tw.initData) : "";
    } catch {
        return "";
    }
}

async function fetchSignalStatusAsync() {
    const base = getSignalsApiBase();
    if (!base) return { ok: false, skipped: true };
    const initData = getTelegramInitData();
    if (!initData) return { ok: false, error: "no_init" };
    try {
        const res = await fetch(`${base}/miniapp/signals/status`, {
            method: "POST",
...
function setupDailySignalLimitBar() {
    const bar = document.querySelector("[data-daily-signal-limit]");
    const textEl = document.querySelector("[data-daily-signal-limit-text]");
    ...
    async function refresh() {
        ...
        const st = await fetchSignalStatusAsync();
        if (!st.ok || !st.data) {
            bar.hidden = true;
            return;
        }
        ...
        const dict = (I18N[currentLang] || I18N.en).trading || {};
        const label = dict.dailyLimitLabel || "Daily limit:";
        textEl.textContent = `${label} ${d.used}/${d.limit}`;
        bar.hidden = false;
    }

    window.jogotoRefreshDailySignalLimit = refresh;
    refresh();
}

async function consumeSignalQuotaAsync() {
    const base = getSignalsApiBase();
    ...
    try {
        const res = await fetch(`${base}/miniapp/signals/consume`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ init_data: initData }),
        });
        ...
        if (data.error === "limit_exceeded") return { ok: false, error: "limit_exceeded" };
        if (res.status === 401 || data.error === "invalid_init_data") {
            return { ok: false, error: "no_init" };
        }
...
function notifySignalQuotaFailure(err) {
    const dict = (I18N[currentLang] || I18N.en).trading || {};
    if (err === "limit_exceeded") {
        showTradingNotice(dict.signalLimitReached || "Signal limit reached.");
    } else if (err === "no_init") {
        showTradingNotice(dict.signalOpenInTelegram || "Open from Telegram.");
    } else {
        showTradingNotice(dict.signalLimitError || "Try again later.");
    }
}
...
function setupTradingForm() {
    ...
    async function rerunSignalInsideModal() {
        if (!signalModal.classList.contains("is-open")) return;

        const quota = await consumeSignalQuotaAsync();
        if (!quota.ok) {
            notifySignalQuotaFailure(quota.error);
            return;
        }
